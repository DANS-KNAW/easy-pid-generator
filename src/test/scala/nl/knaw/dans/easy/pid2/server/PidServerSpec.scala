...
class PidServerSpec extends TestSupportFixture
  ...
  val pidServlet: PidServlet = new PidServlet(new PidGeneratorApp(new PidGeneratorWiring(configuration, databaseAccess)))
  val server: PidServer = new PidServer(8060, pidServlet)
