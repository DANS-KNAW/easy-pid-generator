...
class DatabaseAccess(dbConfig: DatabaseConfiguration) extends DebugEnhancedLogging {
  ...
  private def usernamePasswordCheck: Boolean = {
    (dbConfig.dbUsername, dbConfig.dbPassword) match {
  ...
  protected def createConnectionPool: ConnectionPool = {
    new BasicDataSource {
      this.setDriverClassName(dbConfig.dbDriverClassName)
      this.setUrl(dbConfig.dbUrl)
      dbConfig.dbUsername.foreach(this.setUsername)
      dbConfig.dbPassword.foreach(this.setPassword)
    }
  }
  ...
  def initConnectionPool(): Try[Unit] = Try {
    ...
    logger.info(s"Database connected with URL = ${ dbConfig.dbUrl }, user = ${ dbConfig.dbUsername }")
  }
